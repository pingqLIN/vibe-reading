...
(function () {
  ...
  const RULES = [
    {
      name: '目標語言／locale 明確化',
      test: text => /target language|locale|翻譯成|目標語言/i.test(text),
    },
    {
      name: '忠實保留原意',
      test: text => /preserve.{0,30}(meaning|information)|original meaning|保留.{0,20}(原意|資訊)/i.test(text),
    },
    {
      name: '禁止摘要／增刪／改寫',
      test: text => /do not.{0,60}(summarize|add|omit|reinterpret)|不得.{0,30}(摘要|增|刪|改寫)/i.test(text),
    },
    {
      name: '數字／單位／程式碼等保護',
      test: text => /(numbers?.{0,30}units?|URLs?|code|formulas?|citations?|數字|單位|網址|程式碼|公式|引用)/i.test(text),
    },
    {
      name: 'Glossary／術語一致性',
      test: text => /(glossary|terminology consistent|術語.{0,12}一致|辭庫|詞庫)/i.test(text),
    },
    {
      name: '來源資料／Prompt Injection 邊界',
      test: text => /(untrusted data|never as instructions|不得.{0,20}指令|不要.{0,20}指令)/i.test(text),
    },
    {
      name: '只輸出翻譯結果',
      test: text => /(output only|only.{0,20}translated text|只輸出.{0,20}(翻譯|譯文)|不加任何說明)/i.test(text),
    },
  ];
  ...
  function visibleUnit(unit, mode) {
    return mode === 'line' ? unit.replace(/\n$/, '').trim().length > 0 : !/^\s+$/.test(unit);
  }

  function unitsFor(text, mode) {
    return mode === 'line' ? lineUnits(text) : tokenUnits(text);
  }
  ...
  function render() {
    ...
    const requestedMode = els.diffMode.value;
